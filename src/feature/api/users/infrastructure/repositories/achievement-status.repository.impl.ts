...
@Injectable()
export class AchievementStatusRepositoryImpl implements AchievementStatusRepository{
  constructor(
    ...
    async findAllByUserId(userId: string): Promise<AchievementStatus[]> {
     ...
     async findOneByUserIdAndAchievementId(userId: string, achievementId: number): Promise<AchievementStatus> {
    ...
    async saveOne(userId: string, achievementId: number): Promise<AchievementStatus> {
    ...
    async updateOne(achievementStatus: AchievementStatus): Promise<AchievementStatus> {
      const newAchievementStatus = await this.repository.upsert(this.toEntity(achievementStatus));
      await this.repository.getEntityManager().flush();
